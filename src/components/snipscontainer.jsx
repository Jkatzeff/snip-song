...
export default class SnipsContainer extends React.Component {
	...
	render() {
		...
		const {topSongs, allSnips, handleLike, handleUnlike, createSnip, spotifyWebApi} = this.props;
		return (
			<div>
    			<button onClick={() => {this.show_or_hide()}}>
    				{show ? "Hide top songs?" : "Show top songs?"}
    			</button>
				{show ? <PostNewSnip topSongs={topSongs} createSnip={createSnip} />
								: null}
				<SearchForTrack spotifyWebApi={spotifyWebApi} createSnip={createSnip}/>
